...
/*v1.0.1*/
swfobject.flash = function(url, containerID, width, height, vars) {
	if (!url) {
		throw new Error('The url of the Flash Player is null');
	}
	if (!containerID) {
		throw new Error('the flash containerID is null');
	}
	...
	for (var attr in vars) {
		switch (attr) {
			...
			case "version":
				...
				break;
			case "wmode":
				if (typeof vars[attr] === 'boolean') {
					params[attr] = vars[attr] ? "transparent" : "window";
				} else {
					params[attr] = vars[attr];
				}
				break;
			case "play":
			case "loop":
